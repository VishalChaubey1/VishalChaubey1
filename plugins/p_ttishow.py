...
async def save_group(bot, message):
    ...
    if temp.ME in r_j_check:
        ...
        if message.chat.id in temp.BANNED_CHATS:
            ...
            return
        buttons = [[
            InlineKeyboardButton('𝙷𝙾𝚆 𝚃𝙾 𝚄𝚂𝙴 𝙼𝙴', url=f"https://t.me/PlusTechzBots"),
            InlineKeyboardButton('ʙᴏᴛs | ᴜᴘᴅᴀᴛᴇs', url='https://t.me/PlusTechz')
        ]]
        reply_markup=InlineKeyboardMarkup(buttons)
        ...
        if settings["welcome"]:
            for u in message.new_chat_members:
                ...
                video="https://telegra.ph/file/03691465baa774e46506d.mp4",                                               
                                                 ...
                                                 reply_markup=InlineKeyboardMarkup( [ [ InlineKeyboardButton('↭ ᴍᴏᴠɪᴇs | ᴜᴘᴅᴀᴛᴇs ↭', url='https://t.me/HDFlims4U') ],
                                                                                      [ InlineKeyboardButton('🔥  ↭ʙᴏᴛs | ᴜᴘᴅᴀᴛᴇs↭  🔥', url='https://t.me/PlusTechz') ]
                                                                                    ] )
                )

@Client.on_message(filters.command('ʟᴇᴀᴠᴇ') & filters.user(ADMINS))
async def leave_a_chat(bot, message):
    ...
    try:
        buttons = [[
            InlineKeyboardButton('𝚂𝚄𝙿𝙿𝙾𝚁𝚃', url='t.me/TeamShadowXD')
        ]]
        reply_markup=InlineKeyboardMarkup(buttons)
...
async def disable_chat(bot, message):
    ...
    try:
        ...
        await bot.send_message(
            chat_id=chat_, 
            text=f'<b>ʜᴇʟʟᴏ ғʀɪᴇɴᴅs, \nᴍʏ ᴀᴅᴍɪɴ ʜᴀs ᴛᴏʟᴅ ᴍᴇ ᴛᴏ ʟᴇᴀᴠᴇ ғʀᴏᴍ ɢʀᴏᴜᴘ sᴏ ɪ ɢᴏ! ɪғ ʏᴏᴜ ᴡᴀɴɴᴀ ᴀᴅᴅ ᴍᴇ ᴀɢᴀɪɴ ᴄᴏɴᴛᴀᴄᴛ ᴍʏ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ.</ʙ> \nʀᴇᴀsᴏɴ : <code>{reason}</code>',
            reply_markup=reply_markup)
...
async def re_enable_chat(bot, message):
    ...
    if not sts:
        return await message.reply("ᴄʜᴀᴛ ɪᴅ ɴᴏᴛ ғᴏᴜɴᴅ ɪɴ ᴅʙ !")
...
async def ban_a_user(bot, message):
    ...
    if len(message.command) == 1:
        return await message.reply('ɢɪᴠᴇ ᴍᴇ ᴀ ᴜsᴇʀ ɪᴅ / username')
    r = message.text.split(None)
    if len(r) > 2:
        reason = message.text.split(None, 2)[2]
        chat = message.text.split(None, 2)[1]
...
async def list_chats(bot, message):
    ...
    try:
        ...
        with open('chats.txt', 'w+') as outfile:
            outfile.write(out)
        await message.reply_document('chats.txt', caption="ʟɪsᴛ ᴏғ ᴄʜᴀᴛ")
